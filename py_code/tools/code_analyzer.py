...
def parse_ast_files(
    ...
    if not python_files:
        return {
            "error": f"No Python files found matching pattern: {pattern}",
            "pattern": pattern,
            "root_dir": root_dir
        }
...
def analyze_code_files(
    ...
    summary = {
        "total_files": len(results),
        "total_functions": sum(
            len(result.get("ast_analysis", {}).get("functions", [])) 
            for result in results.values()
        ),
        "total_classes": sum(
            len(result.get("ast_analysis", {}).get("classes", [])) 
            for result in results.values()
        ),
        "total_imports": sum(
            len(result.get("ast_analysis", {}).get("imports", [])) 
            for result in results.values()
        ),
        "total_variables": sum(
            len(result.get("ast_analysis", {}).get("variables", [])) 
            for result in results.values()
        ),
    }
